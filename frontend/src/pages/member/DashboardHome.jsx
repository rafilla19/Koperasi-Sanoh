...
const DashboardHome = () => {
  ...
  const handleDownloadReport = () => {
    ...
    try {
      ...
      doc.text('LAPORAN RINGKASAN KEUANGAN', 105, 30, { align: 'center' });
      ...
      doc.setFontSize(11);
      doc.setTextColor(0);
      doc.text(`Nama Anggota  : ${summary?.full_name || 'Anggota'}`, 20, 45);
      doc.text(`Tanggal Laporan : ${dateStr}`, 20, 52);

      // Savings Section
      doc.setFontSize(14);
      doc.text('1. Ringkasan Simpanan', 20, 70);
      const savingsBody = [
        ['Simpanan Pokok', formatRupiah(summary?.principle_balance)],
        ['Simpanan Wajib', formatRupiah(summary?.mandatory_balance)],
        ['Simpanan Sukarela', formatRupiah(summary?.voluntary_balance)],
        ['Total Saldo', formatRupiah(summary?.total_saving_balance)]
      ];
      autoTable(doc, {
        startY: 75,
      ...
      doc.text('2. Status Pinjaman', 20, finalY);
      const loanBody = [
        ['Pokok Disetujui', formatRupiah(summary?.principal_amount)],
        ['Sisa Saldo', formatRupiah(summary?.total_loan_remaining)],
        ['Angsuran Terbayar', `${summary?.paid_installments} dari ${summary?.total_installments}`],
      ...
      doc.text('3. Transaksi Terbaru', 20, txY);
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`Koperasi Sanoh - Sistem Manajemen Keuangan`, 105, 285, { align: 'center' });
      }
      ...
      alert('Gagal membuat laporan. Pastikan browser Anda mengizinkan unduhan atau coba lagi nanti.');
    }
  };
